...
MASTER_NODE_ADDR="127.0.0.1:9959"                  # Master 节点地址（需全局一致）
export MC_MS_AUTO_DISC=0
START_PORT=13245                                  # 服务起始端口
START_DEVICE=0                                     # 起始 NPU 逻辑设备号
LOG_DIR="log"                                      # 日志目录
NNODES=1                                          # 节点数（当前脚本启动 2 个进程）
export HCCL_IF_BASE_PORT=47788  # HCCL 通信基础端口
TRANSFER_START_PORT=25585
export XLLM_INSTANCE_ID=inst_1 
...
do
  ...
  ./build/xllm/core/server/xllm \
    ...
    --xtensor_master_node_addr=127.0.0.1:19545 \
    --priority_level=3 \
