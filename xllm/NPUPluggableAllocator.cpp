...
// 这个文件在torch_npu github上有，但是镜像打包的的so库没有
...
_AllocationMetadata::_AllocationMetadata(size_t size,
                                         int device_idx,
                                         aclrtStream stream)
...
void NPUPluggableAllocator::set_get_device_stats_fn(
    std::function<c10_npu::NPUCachingAllocator::DeviceStats(int)>
        get_device_stats_fn) {
...
void* NPUPluggableAllocator::malloc(size_t size,
                                    int device,
                                    aclrtStream stream) {
  void* r = alloc_fn_(size, device, stream);
  // 该版本的torch 和 torch_npu 中 Allocator 的 allocate
  // 虚函数是const，与最新版本不一致，此处我去掉了torch allocate的const
  {
...
// 镜像中的torch tensorimpl.h, allocator.h和torch_npu
// NPUPluggabbleAllocator.h的const需要去掉！
...
c10::DataPtr NPUPluggableAllocator::allocate_with_aligned(
  ...
  TORCH_CHECK(false,
              "NPUPluggableAllocator does't has allocate_with_aligned",
...
void* NPUPluggableAllocator::raw_alloc_with_stream(size_t nbytes,
                                                   aclrtStream stream) {
  int device = -1;
  NPU_CHECK_ERROR(c10_npu::GetDevice(&device));
  return malloc(nbytes, device, stream);
...
// 此处是唯一需要侵入修改的地方
// 删除对allocation_metadata的检查

// 我们手动释放torch管理的tensor storage
// 存在next step的激活值覆盖previous step的激活值的情况

// 但是allocation_metadata_是一个unordered_map
// 对于这种覆盖的情况，key相同 allocation_metadata_ do nothing
// 导致metadata对同一个ptr只存了一份metadata

// 当同时释放两个ptr相同的tensor时
// 检查allocation_metadata_的记录会导致第二次释放报错
...
bool NPUPluggableAllocator::initialized() { return initialized_; }
...
void NPUPluggableAllocator::cacheInfo(int dev_id,
...
void NPUPluggableAllocator::recordStream(const c10::DataPtr& ptr,
                                         streamType stream) {
...
void NPUPluggableAllocator::eraseStream(const c10::DataPtr& ptr,
                                        streamType stream) {
  if (erase_stream_fn_) {
...
c10_npu::NPUCachingAllocator::DeviceStats NPUPluggableAllocator::getDeviceStats(
  ...
  if (get_device_stats_fn_) {
    ...
    TORCH_NPU_WARN(
        "get_device_stats_fn_ is not define, please set by "
...
void NPUPluggableAllocator::resetPeakStats(int device) {
  if (reset_peak_status_fn_) {
    ...
    TORCH_NPU_WARN(
        "reset_peak_status_fn_ is not define, please set by "
...
void NPUPluggableAllocator::endAllocateToPool(c10::DeviceIndex device,
                                              c10_npu::MempoolId_t mempool_id) {
  TORCH_CHECK(false,
...
void NPUPluggableAllocator::releasePool(c10::DeviceIndex device,
                                        c10_npu::MempoolId_t mempool_id) {
  TORCH_CHECK(false,
...
void NPUPluggableAllocator::copy_data(void* dest, const void* src, std::size_t
count) const
{
...
NPUPluggableAllocator::getCheckpointState(c10::DeviceIndex device,
...
c10_npu::NPUCachingAllocator::CheckpointDelta
NPUPluggableAllocator::setCheckpointPoolState(
    c10::DeviceIndex device,
