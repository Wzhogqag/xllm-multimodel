...
size_t PrefixCache::insert(const Slice<int32_t>& token_ids,
  ...
  DNodeList node_list;
  Murmur3Key token_hash_key;
  std::vector<Node*> new_nodes_for_global;
  ...
  for (size_t i = 0; i < n_tokens; i += block_size_) {
    ...
    if (iter != cached_blocks_.end()) {
      ...
      if (enable_global_lru_) {
        new_nodes_for_global.push_back(new_node);
      }
  ...
  if (enable_global_lru_) {
    ...
    while (!node_list.is_empty()) {
      ...
      bool is_new = std::find(new_nodes_for_global.begin(),
                              new_nodes_for_global.end(),
                              node) != new_nodes_for_global.end();
      if (is_new) {
